...
async function main(): Promise<void> {
  ...
  // Wire restart: graceful shutdown → spawn start.sh → exit
  // start.sh handles killing old instances, git pull, npm ci, build, and health check
  setRestartHandler(() => {
    ...
    (async () => {
      ...
      // Resolve start.sh relative to the project root
      const projectRoot = path.resolve(import.meta.dirname ?? ".", "..");
      const startScript = path.join(projectRoot, "start.sh");

      // Spawn start.sh detached — it handles git pull, build, and launching a new instance
      // start.sh also kills any remaining bot instances before starting fresh
